...
def dice_loss(
    ...
    sg_score = inputs.detach()
    
    w_ij = torch.maximum(
        torch.tensor(epsilon, device=sg_score.device),
        1 - alpha * (1 - sg_score)
    )

    # w_ij[targets==0] = 1

    numerator = 2 * (w_ij * inputs * targets).sum(-1)
...
def sigmoid_ce_loss(
    ...
    sg_score = inputs.detach().sigmoid()
...
class SetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, mask_type, raw_coordinates):
        ...
        for batch_id, (map_id, target_id) in enumerate(indices):
            ...
            '''
            loss = F.binary_cross_entropy_with_logits(
                map, target_mask, reduction="none"
            )
            pc_loss = loss.mean(0).detach().cpu().numpy()

            alpha=0.6
            epsilon=0.1
            sg_score = map.detach().sigmoid()
            ...
            import numpy as np
            w_ij = w_ij.float().detach().cpu().numpy()
            result = np.full(w_ij.shape[1], 0.1, dtype=w_ij.dtype)
            row_idx, col_idx = np.where(target_mask.cpu().numpy())
            result[col_idx] = w_ij[row_idx, col_idx]

            import pickle
            save_dict = {'coordinates': raw_coordinates, 'loss': pc_loss, 'weight': result}
    ...
    def forward(self, outputs, targets, mask_type, raw_coordinates=None):
        ...
        for loss in self.losses:
            losses.update(
                self.loss_masks(outputs, targets, indices, mask_type, raw_coordinates)
            )
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.losses:
                    l_dict = self.loss_masks(aux_outputs, targets, indices, mask_type, raw_coordinates)
